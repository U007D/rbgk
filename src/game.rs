...
impl Game {
    ...
    pub fn score(&self, rolls: &[u8]) -> Option<u16> {
        ...
        fn inner_score<'a, I>(rolls: I, curr_frame: usize) -> Option<u16> where I: Clone +
                                                                                   Debug +
                                                                                   Iterator<Item = &'a u8> {
            ...
            Some(rolls.clone()
                      ...
                      .map(|&v| u16::from(v))
                      .sum::<u16>()
                + inner_score(rolls.skip(rolls_to_start_of_next_frame), curr_frame + 1).unwrap_or(0))
        }

        inner_score(rolls.iter(), 1)
    }
}
